...
#endif
{
    // Instantiate and add the parameter
    newSliderParameter = new juce::AudioParameterFloat(juce::ParameterID("1",1),"Gain", -1.f, 1.f, 1.f);
    addParameter(newSliderParameter);
}
...
void DemoPluginAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    float gain  = newSliderParameter->get(); //get parameter value
    ...
    for (int channel = 0; channel < totalNumInputChannels; ++channel)
    {
        ...
        for (int currentSample = 0; currentSample < numSamples; currentSample++) {
            *channelData *= gain; //apply gain
            channelData++; //iterate buffer
        }
    }
}
